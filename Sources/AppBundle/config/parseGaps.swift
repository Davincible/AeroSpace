...
struct ResolvedGaps {
    ...
    init(gaps: Gaps, monitor: any Monitor, windowCount: Int = 1) {
        inner = .init(
            vertical: gaps.inner.vertical.getValue(for: monitor, windowCount: windowCount).toPixels(totalDimension: monitor.visibleRect.height),
            horizontal: gaps.inner.horizontal.getValue(for: monitor, windowCount: windowCount).toPixels(totalDimension: monitor.visibleRect.width),
        )

        outer = .init(
            left: gaps.outer.left.getValue(for: monitor, windowCount: windowCount).toPixels(totalDimension: monitor.visibleRect.width),
            bottom: gaps.outer.bottom.getValue(for: monitor, windowCount: windowCount).toPixels(totalDimension: monitor.visibleRect.height),
            top: gaps.outer.top.getValue(for: monitor, windowCount: windowCount).toPixels(totalDimension: monitor.visibleRect.height),
            right: gaps.outer.right.getValue(for: monitor, windowCount: windowCount).toPixels(totalDimension: monitor.visibleRect.width),
        )
    }
}
...
func parsePerMonitorDimensionValues(_ array: TOMLArray, _ backtrace: TomlBacktrace, _ errors: inout [TomlParseError]) -> [PerMonitorValue<DimensionValue>] {
    array.enumerated().compactMap { (index: Int, raw: TOMLValueConvertible) -> PerMonitorValue<DimensionValue>? in
        ...
        return PerMonitorValue(description: monitorDescription, value: dimensionValue, windows: nil, workspace: nil)
    }
}
